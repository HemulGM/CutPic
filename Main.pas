...
begin
  if (AW = 0) or (AH = 0) then
    SLines[1] := 'Холст: Нет изображения'
  else
    SLines[1] := 'Холст: ' + IntToStr(AW) + 'x' + IntToStr(AH);
...
begin
  ...
  BMPJPEG.Free;
  if not IsBreak then
    MessageBox(Handle, 'Задача успешно выполнена!', '', MB_ICONINFORMATION or MB_OK)
  else
    MessageBox(Handle, 'Задача выполнена, но не до конца.' + #13#10 + ' Изображение слишком мало.', '', MB_ICONINFORMATION or MB_OK);
end;
...
begin
  ...
  if IsBreak then
    SLines[5] := 'Статус: Неудача'
  else
    SLines[5] := 'Статус: Успех';
  UpdateInfo;
  MYBMP.Free;
